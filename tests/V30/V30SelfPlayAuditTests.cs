...
namespace TractorGame.Tests.V30
{
    ...
    public class V30SelfPlayAuditTests
    {
        [Fact]
        public void V30_SelfPlay_Audit_10Games()
        {
            var timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            var artifactRoot = TestPathHelper.ResolveFromRepoRoot("artifacts", "ruleai_v30_eval", timestamp);
            Directory.CreateDirectory(artifactRoot);

            var inMemory = new InMemoryLogSink();
            var sink = new CompositeLogSink(
                inMemory,
                new JsonLineLogSink(Path.Combine(artifactRoot, "raw"), "v30-audit"),
                new MarkdownReplayLogSink(Path.Combine(artifactRoot, "replay"), "v30-audit"),
                new AIDecisionBundleLogSink(Path.Combine(artifactRoot, "decision")));
            var logger = new CoreLogger(sink);

            var attemptSummaries = new List<GameAuditSummary>();
            var finishedSummaries = new List<GameAuditSummary>();
            for (var i = 0; i < 30 && finishedSummaries.Count < 10; i++)
            {
                var summary = PlaySingleGame(
                    seed: 13000 + i,
                    label: $"audit10_g{i:D2}",
                    logger: logger);
            ...
            var summaryPath = Path.Combine(artifactRoot, "audit10_summary.json");
            var perTrickPath = Path.Combine(artifactRoot, "audit10_per_trick.md");
            ...
            Assert.Equal(10, finishedSummaries.Count);
        }

        private static GameAuditSummary PlaySingleGame(int seed, string label, IGameLogger logger)
        {
            var roundId = $"v30_{label}_{seed}";
        ...
        private static string BuildSummaryJson(
            ...
            List<GameAuditSummary> summaries,
            List<GameAuditSummary> attempts)
        {
            ...
            var payload = new Dictionary<string, object?>
            {
                ...
                ["discarded_unfinished_attempts"] = attempts
                    .Where(summary => !summary.Finished)
        ...
        private static string BuildPerTrickMarkdown(List<LogEntry> entries, List<GameAuditSummary> summaries)
        {
            var sb = new StringBuilder();
            sb.AppendLine("# RuleAI V30 Audit 10");
            sb.AppendLine();
            ...
            sb.AppendLine($"- play.reject: {entries.Count(entry => string.Equals(entry.Event, "play.reject", StringComparison.Ordinal))}");
            sb.AppendLine();

            foreach (var summary in summaries)
            {
                sb.AppendLine($"## game_{summary.Label}_{summary.Seed} (seed {summary.Seed})");
                sb.AppendLine($"- result: {(summary.Finished ? "finished" : "unfinished")} | winner={summary.WinnerSide} | defender_score={summary.DefenderScore} | rejects={summary.FirstAttemptRejects} | recoveries={summary.FallbackRecoveries}");
                sb.AppendLine();
                ...
                foreach (var trickId in trickIds)
                {
                    ...
                    foreach (var decision in decisions)
                    {
                        ...
                        var decisionText =
                            $"- {turnText} | {decision.Actor} | {ReadString(decision.Payload, "phase")} | {ReadString(decision.Payload, "role")} | {ReadString(decision.Payload, "path")} | {ReadString(decision.Payload, "primary_intent")} | {selectedCards} | {selectedReason}";
        ...
        private static List<TrickPlayLine> ReadTrickFinishCards(Dictionary<string, object?> payload)
        {
            ...
            var result = new List<TrickPlayLine>();
            foreach (var play in element.EnumerateArray())
            {
                if (play.ValueKind != JsonValueKind.Object)
                    continue;

                var playerIndex = play.TryGetProperty("player_index", out var playerEl) && playerEl.TryGetInt32(out var parsed)
                    ? parsed
                    : -1;
                var cards = play.TryGetProperty("cards", out var cardsEl) && cardsEl.ValueKind == JsonValueKind.Array
                    ? cardsEl.EnumerateArray().Select(ReadCardText).Where(text => !string.IsNullOrWhiteSpace(text)).ToList()
                    : new List<string>();
                result.Add(new TrickPlayLine
                {
                    PlayerIndex = playerIndex,
                    Cards = cards
                });
        ...
        private static string FormatCards(IReadOnlyCollection<string> cards)
        {
        ...
        private sealed class GameAuditSummary
        {
